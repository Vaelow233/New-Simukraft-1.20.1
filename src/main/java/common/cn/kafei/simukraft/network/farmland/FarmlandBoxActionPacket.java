...
public record FarmlandBoxActionPacket(BlockPos pos, Action action) {
    ...
    public static void encode(FarmlandBoxActionPacket packet, FriendlyByteBuf buffer) {
        buffer.writeBlockPos(packet.pos());
        buffer.writeEnum(packet.action());
    }

    public static FarmlandBoxActionPacket decode(FriendlyByteBuf buffer) {
        return new FarmlandBoxActionPacket(buffer.readBlockPos(), buffer.readEnum(Action.class));
    }

    public static void handle(FarmlandBoxActionPacket packet, Supplier<NetworkEvent.Context> context) {
    ...
    private static void apply(ServerLevel level, ServerPlayer player, BlockPos pos, Action action) {
        switch (action) {
            ...
            case DEMOLISH -> {
                demolish(level, pos);
                CHANNEL.send(PacketDistributor.PLAYER.with(() -> player), FarmlandBoxOpenResponsePacket.empty(pos));
