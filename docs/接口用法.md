...
```java
public record ExamplePacket(BlockPos pos) {
    public static final Type<ExamplePacket> TYPE = new Type<>(
            ResourceLocation.fromNamespaceAndPath(SimuKraft.MOD_ID, "example"));

    public static final StreamCodec<FriendlyByteBuf, ExamplePacket> STREAM_CODEC =
            StreamCodec.of(ExamplePacket::encode, ExamplePacket::decode);

    public static void encode(FriendlyByteBuf buffer, ExamplePacket packet) {
    ...
    public static void handle(ExamplePacket packet, Supplier<NetworkEvent.Context> context) {
...
```java
CHANNEL.sendToServer(new ExamplePacket(pos));
CHANNEL.send(PacketDistributor.PLAYER.with(() -> player), new ExampleResponsePacket(...));
```
